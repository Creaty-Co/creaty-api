...
class PagesMainMentorView(BaseMainPageView):
    ...
    def patch(self, request, **kwargs):
        ...
        if not PageMentors.objects.filter(page=page, mentor=mentor).exists():
            index = PageMentors.objects.filter(page=page).aggregate(Max('index'))[
                'index__max'
            ]
    ...
    def delete(self):
        ...
        try:
            page__mentor = PageMentors.objects.get(page=page, mentor=mentor)
        except PageMentors.DoesNotExist:
            return
        page__mentor.delete()
        for index in range(page__mentor.index + 1, PageService.MENTORS_COUNT):
            try:
                next_page__mentor = PageMentors.objects.get(page=page, index=index)
            except PageMentors.DoesNotExist:
                break
